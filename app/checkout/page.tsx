...
export default function CheckoutPage() {
  ...
  // ── Auto-calculate pricing whenever address changes ────────────────────────
  useEffect(() => {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      const response = await api.post(
        "/orders/create",
        { deliveryAddress: selectedAddressId, paymentMethod, specialRequests: specialRequests.trim(), useCart: true },
        { withCredentials: true }
      );
      ...
      if (paymentMethod === "online") {
        ...
        const rzp = new (window as any).Razorpay({
          ...
          prefill: { name: selectedAddress?.recipientName || "", contact: selectedAddress?.phoneNumber || "" },
          theme:   { color: "#f97316" },
        });
        ...
        <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                        ...
                        {savedAddresses.map(addr => (
                          <label key={addr._id}
                            ...
                            <input type="radio" name="selectedAddress" value={addr._id}
                              ...
                              onChange={() => { setSelectedAddressId(addr._id); setValidationErrors([]); setError(null); }}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {addr.isDefault && <span className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full font-medium">Default</span>}
                              </div>
            ...
            <div className="md:hidden flex gap-2">
              {currentStep !== "address" && (
                <button onClick={() => { const i = steps.indexOf(currentStep); setCurrentStep(steps[i - 1] as any); }}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 sticky top-4 sm:top-24">
              ...
              {/* Free delivery badge — only show if applicable */}
              ...
              {pricingLoading ? (
                ...
                /* No pricing yet — show subtotal only */
                <div className="space-y-2 mb-4 pb-4 border-b border-gray-200">
                  <div className="flex justify-between text-xs sm:text-sm">
                    <span className="text-gray-600">Subtotal</span>
              ...
              <div className="hidden md:block space-y-3">
                {currentStep !== "review" ? (
                  ...
                  <button onClick={handleCreateOrder}
                    ...
                    className={`w-full py-3 rounded-lg font-semibold text-white transition flex items-center justify-center gap-2 text-sm ${orderCreating || items.length === 0 || !pricing ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-orange-500 to-orange-600 hover:shadow-lg"}`}>
                    {orderCreating
              ...
              {currentStep === "review" && (
                <div className="md:hidden">
                  <button onClick={handleCreateOrder}
                    disabled={orderCreating || items.length === 0 || !pricing}
                    className={`w-full py-2.5 rounded-lg font-semibold text-white transition flex items-center justify-center gap-2 text-xs ${orderCreating || items.length === 0 || !pricing ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-orange-500 to-orange-600 hover:shadow-lg"}`}>
                    {orderCreating
                      ? <><Loader className="w-4 h-4 animate-spin" />Creating...</>
